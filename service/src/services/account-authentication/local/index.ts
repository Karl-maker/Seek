import IAccountAuthentication, { ICredentials, ILoginResponse, ILogoutResponse, IRefreshPayload, ISignupResponse, IRefreshTokenPayload, ILoginOptionals } from "..";
...
export default class LocalAccountAuthentication implements IAccountAuthentication {
    ...
    async login(credentials: ICredentials, details?: ILoginOptionals): Promise<ILoginResponse> {
        ...
        logger.debug("Create tokens after successful credential matching");
    ...
    async refresh(refresh_token: string): Promise<IRefreshPayload> {
        ...
        const account = await this.accountRepository.findById(String(payload.sub));
